...
public static partial class BridgeMod
{
    ...
    /// <summary>
    /// The one live, clickable proceed button anywhere in the scene, or null.
    /// </summary>
    /// <remarks>
    /// Searched from the scene root on purpose. The button does not belong to the
    /// screen that needs it: after combat rewards it lives on the *room*
    /// (<c>NCombatRoom.ProceedButton</c>, and there is an
    /// <c>IRoomWithProceedButton</c> interface for the rest), while the rewards
    /// overlay sits on top of it with no button of its own. Both the state
    /// builder and the proceed action used to call
    /// <c>FindFirst&lt;NProceedButton&gt;(rewardsScreen)</c>, which can only ever
    /// return null - so `can_proceed` was permanently false and `proceed` was a
    /// no-op. Found 2026-09-13 when a run wedged on a fully claimed rewards
    /// screen: nothing left to take, and no way to leave.
    ///
    /// Several rooms keep a hidden button each, so visibility and enablement are
    /// what pick the real one rather than the first match in tree order.
    /// </remarks>
    internal static NProceedButton? FindLiveProceedButton()
    {
        var root = ((Godot.SceneTree)Godot.Engine.GetMainLoop()).Root;
        foreach (var button in FindAll<NProceedButton>(root))
        {
    ...
    /// <summary>A bool property that may not survive the next patch, or null.</summary>
    /// <remarks>
    /// The state builders read a lot of game properties directly, and one that
    /// disappears takes the whole `/state` response down with it - which is
    /// exactly what happened on 2026-09-07 when `CombatManager.IsPlayPhase` was
    /// removed and every read became a 500. New optional fields go through here
    /// so the cost of a rename is that one field reading null.
    /// </remarks>
    internal static bool? SafeBool(Func<bool> getter)
    {
        try { return getter(); }
        catch { return null; }
    }
